...
- GameSaveDemo 适配 Framework `0.6.26` 的 `IUIManager.OnOpenUIViewFail` 事件；最低 Framework 依赖同步提升至 `0.6.26`。

## [0.1.11] - 2026-09-02

### Changed

- GameSaveDemo 移除已废弃的 BestHTTP 热更程序集引用，并同步当前 Framework 配置快照。

## [0.1.10] - 2026-08-28

### Changed

- GameSaveDemo 移除 Launcher Prefab 上重复的 `TextLocalizing` 组件，统一由 Framework Launcher UI 文本渲染链处理。
...
- 修复 GameSaveDemo 覆盖导入或包升级后路径重写遗漏的问题。

## [0.1.7] - 2026-08-20

### Changed

- 更新 GameSaveDemo 文档并移除重复场景监听，改由 Framework `0.6.16` 的统一 `SceneRoute` 接管；最低 Framework 依赖同步提升至 `0.6.16`。

## [0.1.6] - 2026-08-18

### Changed

- 将 Framework 最低依赖提升至 `0.6.15`，并保持 Luban `10.1.0` 与 GameLogin `0.1.8` 的最低依赖，保证独立安装时解析到同一依赖闭环。

## [0.1.5] - 2026-08-18

### Changed

- 将 Luban 最低依赖提升至 `10.0.7`，并将 GameLogin 最低依赖提升至 `0.1.7`，保证独立安装时解析到同一依赖闭环。
...
- 清理 sample asmdef 克隆残留引用：删除代码未使用的 tga / appsflyer / ad 跨包程序集引用（从 MainDemo 模板克隆时带入，GameSaveDemo 实际未调用），避免消费工程未安装这些无关包时编译报 CS0234。
- 补全 sample 依赖声明：package.json dependencies 增加 com.solotopia.nova.framework.kit.network.gamelogin（GameSaveDemo 的登录/绑定/存档演示实际使用），修复消费工程 import sample 后缺对应程序集的编译失败。
