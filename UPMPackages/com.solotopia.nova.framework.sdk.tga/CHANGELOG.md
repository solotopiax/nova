...
- TGADemo 适配 Framework `0.6.26` 的 `IUIManager.OnOpenUIViewFail` 事件；最低 Framework 依赖同步提升至 `0.6.26`。

## [0.1.13] - 2026-09-02

### Changed

- 为 TGA 插件补充静态配置类型元数据；配置 DTO 使用 Nova 自有枚举，避免序列化层直接依赖厂商 API。

## [0.1.12] - 2026-09-01

### Changed

- TGA 属性校验告警补充非法属性的 key 与 value，便于定位上报数据问题。

## [0.1.11] - 2026-08-28

### Changed

- TGADemo 移除 Launcher Prefab 上重复的 `TextLocalizing` 组件，统一由 Framework Launcher UI 文本渲染链处理。

## [0.1.10] - 2026-08-27

### Changed

- TGADemo 在 Network 初始化前完成 Localization 加载与当前语言初始化。

## [0.1.9] - 2026-08-27

### Fixed

- 修复 TGADemo 覆盖导入或包升级后路径重写遗漏的问题。

## [0.1.8] - 2026-08-20

### Changed

- TGADemo 移除重复场景监听，改由 Framework `0.6.16` 的统一 `SceneRoute` 接管；最低 Framework 依赖同步提升至 `0.6.16`。
