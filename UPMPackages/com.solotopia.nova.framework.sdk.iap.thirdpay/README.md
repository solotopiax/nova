...
- 国家码解析不再使用默认国家：`ThirdPayStoreConfig.CountryCode` 和 `IIAPThirdPayCapable.SetDebugCountryCode` 只作为 Debug 覆盖；有效国家码按 `Debug > Lock > Billing > iOS Storefront > AD` 解析，执行 `Trim + ToUpperInvariant`，`IV` / `UNKNOWN` / 空值都视为无国家码；Billing、iOS Storefront 与 AD 国家码会写入 ThirdPay 当前用户存档作为后续兜底，全部缺失时返回空字符串。
...
- Runtime 按 `Data`、`Services/Channel`、`Services/Net`、`Services/Google`、`Services/Order`、`Services/WebView`、`Services/ExternalBrowser`、`Services/Native`、`Utils` 分层；`ThirdPayStore` 按公开入口、非公开方法、字段属性、外部浏览器会话和打点拆分 partial 文件。国家码、商品快照、当前账号存档和外部浏览器 session 分别由专用状态对象维护。
