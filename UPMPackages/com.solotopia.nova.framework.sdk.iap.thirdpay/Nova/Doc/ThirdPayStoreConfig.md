...
| `CountryCode` | Debug 覆盖用 ISO 3166-1 alpha-2 国家/地区代码；生产环境通常留空，让 Store 按 Billing / iOS Storefront / AD 兜底解析，全部缺失时返回空字符串 |
...
| `OpenUrlCmdName` | 解析第三方支付页基址的 NetCmd 名称 |
...
五个 NetCmd 配置字段默认分别为 `ThirdOpenURL`、`ThirdGetProductList`、`ThirdQueryPendingOrder`、`ThirdGetPayChannelParams` 和 `ThirdVerifyIap`，可按项目 NetCmd 表覆盖。其中 `OpenUrlCmdName` 用于解析支付页基址，其余四个字段对应 ThirdPay 业务协议。

国家码不再使用默认国家：`CountryCode` 和业务侧 `SetDebugCountryCode` 都是 Debug 覆盖源，优先级最高；实际支付和商品列表使用的有效国家码按 `Debug > Lock > Billing > iOS Storefront > AD` 解析，全部缺失时返回空字符串。运行时会在各赋值入口对国家码执行 `Trim + ToUpperInvariant`，并将 `IV` / `UNKNOWN` / 空值视为无国家码。iOS 会在 ThirdPay 初始化时通过包内 StoreKit storefront bridge 读取 App Store 国家/地区代码。
...
支付 URL 基址通过 `OpenUrlCmdName` 指定的 NetCmd 解析，默认值为 `ThirdOpenURL`；应用 ID 和渠道统一读取公共请求头的 `AppId` / `Channel`，不在 Store 配置中重复保存。
