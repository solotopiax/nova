...
| 阶段 | 行为 |
|---|---|
| 初始化 | 校验配置 → 解析默认配置文本 → 初始化厂商 DataMaster 本地库（SQLite）→ 订阅登录事件。配置缺失时记 Warning 并降级（读参返回兜底值）。 |
| 登录 | 收到 `UserLogin` → 携带 userId / deviceId / 用户属性向服务端拉取 → 落库 → 触发 `OnConfigRefreshed`。 |
| 释放 | 退订登录事件。厂商单例常驻，无显式 shutdown。 |
...
| `void LogExperimentEvent(string eventName, double value)` | 上报实验指标事件（简化版，自动以当前登录用户与设备构造上下文，无需引用厂商类型）。 |
| `void LogExperimentEvent(string eventName, double value, DMUserContext userContext)` | 上报实验指标事件（高级版，可携带完整画像字段），主数值写入 primaryValue 供服务端聚合。 |
| `void SetUserProperty(string key, object value)` | 设置分流用户属性（如等级 / 国家 / 是否 VIP）；须在登录触发拉取前设置方能参与本次分流。 |
| `void ApplyRequiredUserProperties()` | 一次性设齐两条必传属性 `app_version` / `install_time`，业务无需自行合成。登录拉取前调用一次即可。 |
...
> 两条必传属性的合成 / 记录已内置在插件里，业务**只需登录拉取前调一次 `ApplyRequiredUserProperties()`** 即可，无需自行实现版本号合成或安装时间持久化。需精确安装时间可自行 `SetUserProperty("install_time", 真实值)` 覆盖。
...
// 分流属性（登录前设置）；app_version / install_time 必传，一行设齐
dm.ApplyRequiredUserProperties();          // 内部设 app_version（int）+ install_time（long ms）
dm.SetUserProperty("country_code", "US");  // 其余分流条件按需单独设
...
| 设置分流属性 | `ApplyRequiredUserProperties()` + `SetUserProperty(key, value)` | 一行设齐必传的 **app_version** / **install_time**，再按需设 country_code 等（登录前设置） |
...
| 上报实验事件 | `LogExperimentEvent(eventName, value)` | 计入实验分子 |
