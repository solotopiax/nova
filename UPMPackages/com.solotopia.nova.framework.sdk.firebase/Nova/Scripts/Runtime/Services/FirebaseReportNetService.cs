...
namespace NovaFramework.SDK.FirebasePlugin.Runtime
{
    ...
    /// 封装"上报 Firebase 推送令牌 / Analytics 实例 ID / 国家 / 时区"协议的发送逻辑，
    ...
    public sealed class FirebaseReportNetService
    {
        ...
        /// <param name="country">客户端国家或地区代码；空串表示不更新。</param>
        /// <param name="timezoneOffset">UTC 时区偏移，例如 +08:00；空串表示不更新。</param>
        /// <returns>包含上报响应数据或错误信息的 NetResponse。</returns>
        public async UniTask<NetResponse<PbNetReportFirebaseResp>> Async(string cmdName, string firebasePushToken, string firebaseAnalyticsInstanceId, string country, string timezoneOffset)
        {
