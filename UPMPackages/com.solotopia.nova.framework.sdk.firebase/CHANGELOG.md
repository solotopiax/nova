...
- 默认国家 Topic 只等待广告模块当前运行时国家码，不再依赖已移除的广告国家码本地缓存兜底。
...
- 为 Firebase 插件补充静态配置类型元数据，并明确 WebGL 下仅保留可序列化配置定义。
...
- FirebaseDemo 移除 Launcher Prefab 上重复的 `TextLocalizing` 组件，统一由 Framework Launcher UI 文本渲染链处理。

## [0.1.9] - 2026-08-27

### Changed

- FirebaseDemo 在 Network 初始化前完成 Localization 加载与当前语言初始化。
