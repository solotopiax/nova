...
- 为 Ad 插件补充静态配置类型元数据，使 Framework 可在构造插件前完成启用过滤。

## [1.1.9] - 2026-08-28

### Changed

- AdDemo 移除 Launcher Prefab 上重复的 `TextLocalizing` 组件，统一由 Framework Launcher UI 文本渲染链处理。

## [1.1.8] - 2026-08-27

### Changed

- AdDemo 在 Network 初始化前完成 Localization 加载与当前语言初始化。

## [1.1.7] - 2026-08-27

### Fixed

- 修复 AdDemo 覆盖导入或包升级后路径重写遗漏的问题。
...
- AdDemo 移除重复场景监听，改由 Framework `0.6.16` 的统一 `SceneRoute` 接管；最低 Framework 依赖同步提升至 `0.6.16`。

## [1.1.4] - 2026-08-18

### Added

- 新增广告国家码统一获取能力：渠道初始化后可返回国家或地区代码，广告聚合层负责等待、超时和上次成功缓存兜底，并把空值或 `IV` 统一视为不可用。
- 新增广告国家码等待超时配置，默认 5 秒；其他 SDK 可异步获取最终国家码，无需自行轮询广告数据或使用系统区域兜底。

### Changed

- 将 Framework 最低依赖提升至 `0.6.15`，并同步本轮 Sample ConfigMaster 的国家码等待默认值。

## [1.1.3] - 2026-08-13

### Changed

- 将 Framework 最低依赖提升至 `0.6.10`，使广告基础能力解析到本轮统一契约。
