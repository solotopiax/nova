...
- Mobile 的订单号、验单状态和失败原因字段改由 `MobileStore.Track.cs` 直接构造，继续保持既有 `nova_order_id` 语义。
- 最低 IAP Core 依赖提升至 `0.1.16`，使用新的通用打点属性与事件发送入口。

### Fixed

- 平台已有购买与权益查询并发时，权益恢复会等待 receipt 缓存完成再生成验单记录，并从 `Entitlement.Order` 兜底缓存票据，避免已拥有的非消耗品因缺少 Google token 跳过验单并派发空恢复事件。
- 登录补单早于 Unity IAP 商店连接完成时会保留待刷新权益，并在商店和商品就绪后自动补跑；平台已有购买拉取失败时也会通过统一入口执行服务端、本地和权益兜底，避免后台连接时序造成启动补单缺口。
- Unity IAP 商店连接改由 Store 运行期后台任务等待，并接入 Dispose 取消令牌，避免平台连接长期无回调时阻塞游戏主 Loading。
- 用户取消或平台拒绝活跃支付时，失败回调改为交给 `PayAsync` 返回边界统一打点，避免同一次失败重复发送 `nova_iap_local_pay_fail`；无返回链路的迟到回调仍保留一次兜底记录。
- Android 验单成功打点改用 Google 订单号持久化去重，并在再次遇到历史订单时迁移旧版 purchase token 记录，避免存档继续保存 token 作为打点身份。
