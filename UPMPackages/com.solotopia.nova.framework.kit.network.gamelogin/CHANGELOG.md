...
- GameLoginDemo 适配 Framework `0.6.26` 的 `IUIManager.OnOpenUIViewFail` 事件；最低 Framework 依赖同步提升至 `0.6.26`。
...
- GameLoginDemo 移除 Launcher Prefab 上重复的 `TextLocalizing` 组件，统一由 Framework Launcher UI 文本渲染链处理。

## [0.1.11] - 2026-08-27

### Changed

- GameLoginDemo 在 Network 初始化前完成 Localization 加载与当前语言初始化。

## [0.1.10] - 2026-08-27

### Fixed

- 修复 GameLoginDemo 覆盖导入或包升级后路径重写遗漏的问题。

## [0.1.9] - 2026-08-20

### Changed

- 更新 GameLoginDemo 文档并移除重复场景监听，改由 Framework `0.6.16` 的统一 `SceneRoute` 接管；最低 Framework 依赖同步提升至 `0.6.16`。

## [0.1.8] - 2026-08-18

### Changed

- 将 Framework 最低依赖提升至 `0.6.15`，并将 Luban 最低依赖保持在 `10.1.0`，与当前 UPM 分发版本保持一致。

## [0.1.7] - 2026-08-18

### Changed

- 将 Luban 最低依赖提升至 `10.0.7`，与官方 Luban v4.11.0 工具包保持一致。
