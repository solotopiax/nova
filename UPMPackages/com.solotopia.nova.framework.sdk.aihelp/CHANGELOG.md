...
- AIHelpDemo 适配 Framework `0.6.26` 的 `IUIManager.OnOpenUIViewFail` 事件；最低 Framework 依赖同步提升至 `0.6.26`。

## [0.0.13] - 2026-09-02

### Changed

- 为 AIHelp 插件补充静态配置类型元数据，并收紧不支持平台的程序集编译边界。

## [0.0.12] - 2026-08-28

### Changed

- AIHelpDemo 移除 Launcher Prefab 上重复的 `TextLocalizing` 组件，统一由 Framework Launcher UI 文本渲染链处理。
