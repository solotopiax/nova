...
三方登录只读取既有绑定关系，不产生绑定副作用；未绑定返回 `10404`，业务应以 `Login.Async(string.Empty, string.Empty, false)` 按 DeviceID 优先登录已有游客、不存在才创建，然后再走 GameBind。`forceNewAccount=true` 表示强制新建游客并可能软删同设备旧游客，不属于这条自动回退流程。网络错误、服务端错误、封禁、设备标识缺失等其他失败也不得自动转入游客注册。
...
| `10011` | 账号已删除 | 清理当前会话 |
| `10404` | OpenID 未绑定 | 以 `forceNewAccount=false` 按 DeviceID 登录或创建游客，再走绑定 |
...
| 错误码 | 含义 | 业务动作 |
|---:|---|---|
| `10401` | 目标 OpenID 已被其他 UID 占用 | 不覆盖绑定，按产品策略引导 |
...
| `10408` | 当前 UID 已绑定其他 OpenID | 不重复绑定；继续使用当前绑定或按产品策略更换账号 |
...
当前账号
  ...
  -> Bind 成功 -> 已绑定
  -> Bind 10402 -> 冲突待选择
  -> Bind 10401 -> 目标 OpenID 已被占用，保持当前账号
  -> Bind 10408 -> 当前 UID 已有其他绑定，保持当前账号
  -> 受保护接口 10400 -> 未登录
