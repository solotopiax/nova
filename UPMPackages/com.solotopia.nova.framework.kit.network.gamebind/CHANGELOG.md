...
- GameBindDemo 适配 Framework `0.6.26` 的 `IUIManager.OnOpenUIViewFail` 事件；最低 Framework 依赖同步提升至 `0.6.26`。

## [0.1.5] - 2026-09-02

### Changed

- GameBindDemo 移除已废弃的 BestHTTP 热更程序集引用，并同步当前 Framework 配置快照。

## [0.1.4] - 2026-08-28

### Changed

- GameBindDemo 移除 Launcher Prefab 上重复的 `TextLocalizing` 组件，统一由 Framework Launcher UI 文本渲染链处理。

## [0.1.3] - 2026-08-27

### Changed

- GameBindDemo 在 Network 初始化前完成 Localization 加载与当前语言初始化。

## [0.1.2] - 2026-08-27

### Fixed

- 修复 GameBindDemo 覆盖导入或包升级后路径重写遗漏的问题。
...
- 将 Framework 最低依赖提升至 `0.6.15`，并同步 Luban `10.1.0`、GameLogin `0.1.8` 与 GameSave `0.1.6` 最低依赖，保证独立安装时解析到同一依赖闭环。

## [0.0.13] - 2026-08-18

### Changed

- 将 Luban 最低依赖提升至 `10.0.7`，并同步提升 GameLogin `0.1.7` 与 GameSave `0.1.5` 最低依赖，保证独立安装时解析到同一依赖闭环。
