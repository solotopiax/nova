...
- IAPDemo 适配 Framework `0.6.26` 的 `IUIManager.OnOpenUIViewFail` 事件；最低 Framework 依赖同步提升至 `0.6.26`。
...
- IAPDemo 移除 Launcher Prefab 上重复的 `TextLocalizing` 组件，统一由 Framework Launcher UI 文本渲染链处理。

## [0.1.9] - 2026-08-27

### Changed

- IAPDemo 在 Network 初始化前完成 Localization 加载与当前语言初始化。

## [0.1.8] - 2026-08-27

### Fixed

- 修复 IAPDemo 覆盖导入或包升级后路径重写遗漏的问题。

## [0.1.7] - 2026-08-26

### Changed

- 优化 SKU Excel 导入、导出目录记忆、覆盖确认与商品列表检索布局。

## [0.1.6] - 2026-08-20

### Changed

- IAPDemo 移除重复场景监听，改由 Framework `0.6.16` 的统一 `SceneRoute` 接管；最低 Framework 依赖同步提升至 `0.6.16`。
