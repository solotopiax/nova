 ...
 *            ② 询问是否将本 Demo 的 GoogleSigninDemo 场景设为 Build Settings 启动场景。
...
{
    /// <summary>
    /// Demo 样例兜底守卫。检测旧版本 Demo 并询问清理，提示是否将 GoogleSigninDemo 设为启动场景。
    /// </summary>
    [InitializeOnLoad]
    internal static class SampleVersionGuard
    {
        ...
        /// Demo 子目录名。
        /// </summary>
        private const string c_DemoSubdir = "GoogleSigninDemo";

        /// <summary>
        /// 启动场景文件名（含扩展名）。
        /// </summary>
        private const string c_GoogleSigninDemoSceneName = "GoogleSigninDemo.unity";
        ...
        /// <summary>
        /// 枚举 Samples 根下所有含 GoogleSigninDemo 子目录的版本号，按 semver 降序返回。
        /// </summary>
        ...
        /// <summary>
        /// 检查 Build Settings 第一项是否为本 Demo 的 GoogleSigninDemo 场景，否则弹窗询问设为启动场景。
        /// </summary>
        /// <param name="selfDemoDir">本 Demo 目录路径。</param>
        private static void CheckStartupScene(string selfDemoDir)
        {
            string targetScenePath = $"{selfDemoDir}/{c_GoogleSigninDemoSceneName}";
            ...
            bool apply = EditorUtility.DisplayDialog(
                "Nova Framework Demo",
                $"是否将本 Demo 的 GoogleSigninDemo 场景设为启动场景?\n\n目标: {targetScenePath}\n(会替换 Build Settings 第一项)",
                "设为启动场景",
                "稍后");
