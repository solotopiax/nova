...
namespace NovaFramework.Sdk.Appsflyer.Samples.Runtime
{
    ...
    {
        private int m_DemoAppsflyerViewSerialID = -1;
        ...
        protected override void OnEnter(ProcedureOwner procedureOwner)
        {
            ...
            m_DemoAppsflyerViewSerialID = Nova.UI.OpenUIViewAsync<DemoAppsflyerView>();
            if (m_DemoAppsflyerViewSerialID < 0)
            {
                Log.Error(LogTag.UI, "ProcedurePlaying — DemoAppsflyerView 打开请求失败。");
            }
        }

        private void OnOpenUIViewFail(int serialID, string assetLocation, string errorMessage)
        {
            if (serialID != m_DemoAppsflyerViewSerialID)
            {
                return;
            }

            m_DemoAppsflyerViewSerialID = -1;
            Log.Error(LogTag.UI, "ProcedurePlaying — DemoAppsflyerView 异步打开失败。Asset 地址 '{0}'：{1}", assetLocation, errorMessage);
        }
        ...
        {
            Nova.UI.OnOpenUIViewFail -= OnOpenUIViewFail;
            if (m_DemoAppsflyerViewSerialID >= 0 && Nova.UI.IsLoadingUIView(m_DemoAppsflyerViewSerialID))
            {
                Nova.UI.CloseUIView(m_DemoAppsflyerViewSerialID);
            }
