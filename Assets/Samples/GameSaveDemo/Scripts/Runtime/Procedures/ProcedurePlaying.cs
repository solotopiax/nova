...
namespace NovaFramework.Kit.Network.GameSave.Samples.Runtime
{
    ...
    {
        private int m_DemoGameSaveViewSerialID = -1;
        ...
        protected override void OnEnter(ProcedureOwner procedureOwner)
        {
            ...
            m_DemoGameSaveViewSerialID = Nova.UI.OpenUIViewAsync<DemoGameSaveView>();
            if (m_DemoGameSaveViewSerialID < 0)
            {
                Log.Error(LogTag.UI, "ProcedurePlaying — DemoGameSaveView 打开请求失败。");
            }
        }

        private void OnOpenUIViewFail(int serialID, string assetLocation, string errorMessage)
        {
            if (serialID != m_DemoGameSaveViewSerialID)
            {
                return;
            }

            m_DemoGameSaveViewSerialID = -1;
            Log.Error(LogTag.UI, "ProcedurePlaying — DemoGameSaveView 异步打开失败。Asset 地址 '{0}'：{1}", assetLocation, errorMessage);
        }
        ...
        {
            Nova.UI.OnOpenUIViewFail -= OnOpenUIViewFail;
            if (m_DemoGameSaveViewSerialID >= 0 && Nova.UI.IsLoadingUIView(m_DemoGameSaveViewSerialID))
            {
                Nova.UI.CloseUIView(m_DemoGameSaveViewSerialID);
            }
