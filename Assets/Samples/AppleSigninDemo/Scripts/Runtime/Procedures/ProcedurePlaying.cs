...
namespace NovaFramework.Sdk.Applesignin.Samples.Runtime
{
    ...
    {
        private int m_DemoAppleSigninViewSerialID = -1;
        ...
        protected override void OnEnter(ProcedureOwner procedureOwner)
        {
            ...
            m_DemoAppleSigninViewSerialID = Nova.UI.OpenUIViewAsync<DemoAppleSigninView>();
            if (m_DemoAppleSigninViewSerialID < 0)
            {
                Log.Error(LogTag.UI, "ProcedurePlaying — DemoAppleSigninView 打开请求失败。");
            }
        }

        private void OnOpenUIViewFail(int serialID, string assetLocation, string errorMessage)
        {
            if (serialID != m_DemoAppleSigninViewSerialID)
            {
                return;
            }

            m_DemoAppleSigninViewSerialID = -1;
            Log.Error(LogTag.UI, "ProcedurePlaying — DemoAppleSigninView 异步打开失败。Asset 地址 '{0}'：{1}", assetLocation, errorMessage);
        }
        ...
        {
            Nova.UI.OnOpenUIViewFail -= OnOpenUIViewFail;
            if (m_DemoAppleSigninViewSerialID >= 0 && Nova.UI.IsLoadingUIView(m_DemoAppleSigninViewSerialID))
            {
                Nova.UI.CloseUIView(m_DemoAppleSigninViewSerialID);
            }
