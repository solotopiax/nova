...
namespace NovaFramework.Sdk.IAP.Samples.Runtime
{
    ...
    {
        private int m_DemoIAPViewSerialID = -1;
        ...
        protected override void OnEnter(ProcedureOwner procedureOwner)
        {
            ...
            m_DemoIAPViewSerialID = Nova.UI.OpenUIViewAsync<DemoIAPView>();
            if (m_DemoIAPViewSerialID < 0)
            {
                Log.Error(LogTag.UI, "ProcedurePlaying — DemoIAPView 打开请求失败。");
            }
        }

        private void OnOpenUIViewFail(int serialID, string assetLocation, string errorMessage)
        {
            if (serialID != m_DemoIAPViewSerialID)
            {
                return;
            }

            m_DemoIAPViewSerialID = -1;
            Log.Error(LogTag.UI, "ProcedurePlaying — DemoIAPView 异步打开失败。Asset 地址 '{0}'：{1}", assetLocation, errorMessage);
        }
        ...
        {
            Nova.UI.OnOpenUIViewFail -= OnOpenUIViewFail;
            if (m_DemoIAPViewSerialID >= 0 && Nova.UI.IsLoadingUIView(m_DemoIAPViewSerialID))
            {
                Nova.UI.CloseUIView(m_DemoIAPViewSerialID);
            }
