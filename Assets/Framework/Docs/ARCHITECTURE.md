...
| `Nova.Native` | `NativeComponent` | 系统通知授权、应用设置与应用内评价请求桥接 |
...
- `NativeComponent` / `INativeManager` 负责通知授权状态、业务显式请求、应用设置跳转与应用内评价请求；框架启动不会自动发起系统提示。
- Native 不负责 APNs / FCM Token、远程消息接收或业务通知策略；这些仍属于对应 SDK 或业务层。
- 应用内评价只返回“已交给系统”的请求状态，不代表系统展示、用户评价或提交完成；触发时机与频率属于业务层。
- Android 构建声明 `POST_NOTIFICATIONS`，iOS 构建链接 `UserNotifications.framework` 与 `StoreKit.framework`；不因此配置 Push capability 或 APNs entitlements。
...
- Asset/Prefab、Procedure/HybridCLR、Config/SDK、Native/平台原生能力是当前需明确边界的模块组合。
