...
Nova 全局配置窗口，三段式布局（顶栏 + 左树 + 右面板），集中管理 ConfigMasterSO 的 Platform×Channel 矩阵编辑、SDK Plugin 配置、AppConfigs 与 Custom 参数填写、HybridCLR DLL 配置、CDN 内容部署及 Luban/Python3 环境检查。Platform、Channel 与 DevelopMode 均可在窗口内切换；编辑平台是窗口本地状态，不写入 ConfigMasterSO。编辑平台与 Unity Active BuildTarget 一致时才允许导出 ConfigRuntimeSO 并使对应 YooAsset 配置生效。
...
| `m_CleanCdnRemoteBeforeDeploy` | `bool` | `false` | 资源部署前是否清理本次上传计划中的精确文件；仅窗口会话状态，不写 Config |
| `m_IsCdnWhitelistDeploying` | `bool` | `false` | 白名单配置及版本文件部署是否执行中；控制白名单部署按钮禁用与重复点击保护 |
| `m_CleanCdnWhitelistRemoteBeforeDeploy` | `bool` | `false` | 白名单部署前是否清理本次上传计划中的精确文件；仅窗口会话状态，不写 Config |
...
// Play Guard 配置异常导航入口：切换来源资产、编辑平台、Channel/DevelopMode 并打开对应面板
...
这些导航入口使用 `platform` 参数定位启动 Guard 报告的来源坐标，但不会改写 ConfigMaster 平台。即使请求平台与 Unity 当前 Active BuildTarget 不一致，窗口仍会打开正确配置份供编辑和保存；顶栏以红色错误文字提示不一致，并阻止 ConfigRuntime 导出与所选 YooAsset 配置生效。
...
```
OnEnable()
  ├─ EnsureEditingPlatform() → 首次优先 Active BuildTarget，未映射时回退 Android
  ...
  └─ EnsureStyles()（懒初始化 GUIStyle）
     └─ DisabledScope(isPlayingOrWillChangePlaymode)
           └─ DrawBody()
                ...
                ├─ DrawTopBar()      顶部工具栏（SO 选择 + 编辑 Platform + Channel/DevelopMode + 导出门禁）
                ├─ HorizontalScope:
                │    ├─ DrawLeftTree()   左树（三组：环境检测 / 通用配置 / SDK）
                │    └─ DrawRightPanel() 右面板（分发到具体 Panel）
                ├─ ApplyPendingCoordSwitch()   延迟切换 Platform / Channel / DevelopMode：消费 TryApply 记录的 pending，让数字字段在旧坐标格子下完成失焦提交后再换坐标（PAT-22 升级）
...
第二行（仅 m_Master != null 时，labelWidth=80f）：
  Label("平台类型：", 64f) + IntPopup(非 None PlatformType，120f，窗口本地编辑坐标) + Space(24f)
  Label("渠道类型：", 64f) + IntPopup(非 None ChannelType 声明名，120f) + Space(24f)
  ...
  DisabledScope(编辑平台 != Active BuildTarget 平台) → SuccessButton("导出", 64f)
...
`PlatformType.None` 与 `ChannelType.None` 都是内部哨兵，不在下拉框显示，也不能作为 Config 编辑、保存或导出坐标。旧资产绑定窗口时会删除 Platform None 行；Channel None 行在同平台没有 Official 行时迁移为 Official，否则删除，并将当前渠道 None 归一为 Official。
渠道下拉直接显示 `ChannelType` 的声明名，不执行 Unity 的驼峰拆词，因此显示为 `WeChat`、`TikTok`。
...
- “热更资源-云端目录位置”中的固定前缀只读显示，可编辑输入框只负责 `RemotePathSuffix`；后缀支持同样四项占位符，部署构建 Object Key 前解析。紧随两项之后的 HelpBox 说明热更资源部署作用及占位符。`{Platform}` = ConfigWindow 当前编辑平台，`{Channel}` = `ConfigMasterSO.CurrentChannel`，`{Package}` = Nova.prefab 上 AssetComponent 的默认资源包名（空时回退包列表首项），`{Version}` = `Application.version`。目标 Object Key 为“固定前缀 + 已解析后缀 + 本地相对文件路径”。部署只上传计划内文件；同 Key 由 OSS 覆盖，其他云端对象保持不变，不在上传前执行删除。
- “白名单部署”位于资源部署与 Cloudflare 缓存清理之间。设备 ID 以可增删字符串数组编辑，部署时去空、Trim、按首次出现顺序去重并生成 JSON 根数组 `VersionsCheckWhiteList.json`。三个版本文件上方有独立的“自动关联最新版本”开关，文字使用与下方字段一致的标签列宽，勾选框对齐下方输入框的值列起点；开关下方 HelpBox 说明 `.bytes` 锚点、三文件自动刷新以及 ConfigMaster 当前维度取值规则。开关随 CDN 当前维度保存；其 Platform 维度取 ConfigWindow 编辑平台。开启时以已配置 `.bytes` 路径的父目录为锚点寻找最新完整版本，根据当前 ConfigMaster 当前维度 YooAsset 配置中的 `PackageFilePrefix`、默认包名和资源版本自动生成匹配的 `.bytes/.hash/.version` 路径，不读取 `YooAssetSettings.asset` 的实际值。三项只读展示且部署前重新解析；`.bytes` 的“选择”仍可改变锚点，`.hash/.version` 的“选择”禁用。关闭后恢复三项手工编辑与选择，并分别实时校验文件非空、位于 Unity 项目根内、实际存在、不经过 symlink/junction 且扩展名匹配；每项独立标红并在其下方显示包含实际路径的错误提示。配置文件使用完整的“`PresetOSSPath + 配置文件云端文件位置`”，三个版本文件上传到“`PresetOSSPath + 版本文件云端目录位置`”，各路径均支持四类占位符。配置文件位置为空、不是 `.json` 文件、使用绝对 URL、父级路径或含查询/片段时仅跳过配置文件，不回退到版本文件目录；三个版本文件仍按版本目录独立部署。该按钮使用独立上传计划，不会改变资源部署的文件集合，同样只上传并按 Key 覆盖。
- Cloudflare 页面配置 `ZoneID`、Bearer `API Token` 与缓存 URL 列表；字段下方、执行按钮上方的 HelpBox 说明清除缓存的用途、分隔与去重规则、每批最多 100 条的顺序发送和 Token 权限。请求地址内部固定构造为 `https://api.cloudflare.com/client/v4/zones/{ZONE_ID}/purge_cache`。输入框提示多个路径使用英文逗号分隔，解析同时兼容英文逗号、英文分号和换行，按首次出现顺序去重，并按每 100 条顺序发送。API Token 需要 `Zone -> Cache Purge` 权限。
- 三个按钮分别触发资源部署、白名单部署和缓存清理，首个失败即停止。本地文件或目录不存在、越界或为空时，对话框和日志会明确显示解析后的错误路径。资源与白名单部署均不清理云端对象；上传失败时停止后续文件。Secret/Token 在界面中遮罩，错误信息进入日志或对话框前脱敏，但配置资产中的序列化值仍是明文。
...
```
OnClickExport()
  1. 编辑平台必须与 ActivePlatform.Current 一致，否则显示错误并 return
  2. Validator.Validate(master, m_EditingPlatform, master.CurrentChannel, master.CurrentDevelopMode)
