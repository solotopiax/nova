...
HybridCLR 面板全部字段的维度 Override 单项（仅 Editor 期消费）；对应 `ConfigMasterSO` 的 `AotMetadataDlls` / `GameDlls` / `LinkXmlTargetPath` / `GameEntranceProcedureName` 四个字段。当 `HybridEditorConfigsMask` 勾选维度轴后，列表中与当前维度匹配的首个条目覆盖上述顶层字段；无命中时回落顶层字段值。
