...
Asset 模块编辑期注入层；按 ConfigMasterSO 中显式声明的路径字段注入 `YooAssetSettings` 与加载 `BundleCollectorSetting`，替代 `Resources.Load` / `AssetDatabase.FindAssets` 全工程扫描，根除多 Sample 共存时命中错副本问题。Sample 不再常驻 `Resources/YooAssetSettings.asset`；Editor Play Mode 会在 `BeforeSceneLoad` 按当前三维坐标重新注入，其中 Platform 实时映射 Unity Active BuildTarget；Player 构建则由构建期 staging 临时提供运行时副本。
...
```csharp
// 按 ConfigMasterSO 当前三维坐标解析并注入 YooAssetSettings 到 YooAssetConfiguration 静态全局；Platform 实时映射 Unity Active BuildTarget
...
// 按 ConfigMasterSO 当前三维坐标解析并加载 BundleCollectorSetting；Platform 实时映射 Unity Active BuildTarget
...
| `EditorUtil.SceneRoute` | `EditorSceneManager.sceneOpened` 的 `OpenSceneMode.Single` | 解析 active ConfigMaster 一次，依次输出 Scene / ConfigMaster / YooAssetSettings / PipifySettings 四条摘要，并调用 `YooAssetInjector.Inject`。Nova 源码开发仓切换 Sample 时，还会把 Build Settings 收敛到该 Sample 的唯一入口场景；消费态不自动修改。Additive 打开与 Player 构建内部场景加载不切换路由；相同场景重开仅重新确认和注入，不会为了日志额外写入 `Globals.json` |
