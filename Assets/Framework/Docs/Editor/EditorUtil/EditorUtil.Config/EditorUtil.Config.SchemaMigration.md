...
- `TryMigrateAndReexport`：先验证当前导出坐标和目标路径，再迁移、保存 Master，并覆盖关联的 Runtime 快照。平台坐标实时取 Unity Active BuildTarget；未映射为 Android / iOS / WebGL 时不重导出并返回明确错误。
...
- Runtime 重导出失败会返回错误；导出前置条件在推进 Master 版本前检查。当前 Unity Active BuildTarget 未映射为 Nova PlatformType 时同样失败，不回退到升级前资产保存的平台值。
