...
本地化字体 Excel 使用 Luban 原生表结构，不需要文本导出的多语言 PreFilter；但它与其他非 Table 模块一样，先在源目录的 `_temp/_publish` 完成生成和验证，再一次性发布正式 JSON/C#。
...
校验 FontUnitsSettings、源目录和输出路径
  ...
  -> 克隆字体 Settings，把 JSON/C# 输出改写到 _temp/_publish
  -> 仅代码导出时复制正式 JSON，供 Map 属性生成读取
