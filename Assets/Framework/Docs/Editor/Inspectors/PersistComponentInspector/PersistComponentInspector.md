...
SQLite 管理器        TypesSelector → ISQLiteManager 实现类（WebGL 下保留展示但禁用）
...
SQLite 启用 AES 加密        Toggle（非 WebGL 切换时立即批量迁移；WebGL 下保留展示但禁用编辑）
SQLite Cipher 密码          TextField（临时缓冲，与序列化值不同步时激活"存档转换"；WebGL 下保留展示但禁用编辑）
存档转换                    Button（用新密码删旧库、按原数据建新库，AES 条目状态保持不变；WebGL 下保留展示但禁用）
HelpBox(Info)              AES 条目级加密说明 + Cipher 数据库级加密说明
─────────────────────────────────────────────────────────
[自动保存配置区]
PlayerPrefs 自动保存间隔(秒)   Property
FileFragment 自动保存间隔(秒)  Property
SQLite 自动保存间隔(秒)        Property（WebGL 下保留展示但禁用）
...
Editor 模式（非 Play）：
  ...
  [SQLite（Editor）(n) ▼]        分类折叠 + 存储项搜索 + 打开文件夹 + 编辑/保存/清除 + 清除全部（WebGL 下保留折叠区但禁用）
                                 ├─ Windows：可视化工具 / 应用预览
...
**WebGL 平台**：SQLite 存储实现在 WebGL 上以静默空操作运行，Initialize 输出警告，Get 返回默认值，Set 被忽略；Editor 面板保持与 Android / iOS 相同的 SQLite 区块可见性，管理器、AES、Cipher 密码、存档转换、自动保存和数据区等暂不支持的交互统一置灰，不执行存档迁移或本地库操作。
