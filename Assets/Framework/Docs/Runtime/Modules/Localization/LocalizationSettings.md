...
- 模块统一数据格式 `DataFormat`，默认 `Json`，可选 `Binary`
...
- `LocalizationComponent.Start()` 会把 `DataFormat` 和两组单元列表传入 `LocalizationManagerConfig`。
...
- 文本、字体和支持语言列表必须保持相同格式，路径后缀分别使用 `.json` 或 `.bytes`。
