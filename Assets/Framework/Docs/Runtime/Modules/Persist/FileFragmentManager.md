...
它把每个 `classify` 对应成一个 `.dat` 二进制文件，可选 AES 加密。运行时核心不是“整库常驻”，而是“目录扫描 + 分类懒加载 + 脏片段写回”。
...
- 确保目录存在
- 找出所有 `.dat` 文件
- 同时发现缺少正式文件的 `.dat.bak`，用于恢复写入中断现场
- 过滤掉已加载分类
- Android、iOS 与 Editor 在线程池并行反序列化
- WebGL 在主线程逐个反序列化，并在每个文件之间让出一帧；不会把任务投递到 WebGL 不可用的线程池
...
3. 每个分类先写同目录临时文件并回读校验
4. 将上一版正式文件保留为 `.bak`，再提升新文件
5. 成功的分类才从脏集合移除

失败的分类会保留在脏集合里，等待后续重试。

WebGL 下文件先写入浏览器内存中的虚拟文件系统；正式文件完成替换后，框架请求 `FS.syncfs(false)` 将变更提交到 IndexedDB。底层会串行执行并合并重叠同步请求，失败时输出明确错误，避免连续文件操作并发触发多个 `syncfs`。受浏览器异步存储机制限制，同步 `Save()` 表示文件写入和校验完成且持久化请求已发起，不表示 IndexedDB 回调已经返回。

微信小游戏等不提供 IndexedDB 的 WebGL 宿主会跳过 `FS.syncfs`，避免宿主错误分支终止 Unity 主循环。此时 FileFragment 仍可在当前运行会话的虚拟文件系统中读写，但不保证关闭或重启小游戏后保留；需要跨启动持久化的数据应改用宿主支持的存储能力。
...
真正删文件发生在下一次 `Save()`。
删除时会同时清理正式文件、`.bak` 和残留 `.tmp`，避免后续启动从备份恢复已经明确删除的数据；删除失败的分类会保留在队列中等待下次保存重试。
...
- 如果正式分片损坏，系统会先尝试 `.bak`；恢复成功会重建正式文件，恢复失败则把坏文件隔离为 `.corrupt.<UTC 时间>` 并以空数据运行。
- 解密或格式损坏不会阻断其他分类和 Persist 初始化；文件系统不可读、无权限等运行环境错误仍会向上抛出，避免伪装成新存档。
- 文件分片并不意味着跨分类一致性事务；多个分类的保存是逐片段处理的。
- WebGL 首次没有分片文件时会立即完成；已有分片时按文件逐帧恢复，文件数量或单文件体积过大仍会延长启动时间。
- WebGL 关闭页面时浏览器不保证尚未完成的异步 IndexedDB 操作继续执行；关键数据应在状态变化时及时 `Save()`，不要只依赖页面关闭阶段保存。
- 微信小游戏等无 IndexedDB 的 WebGL 宿主只提供会话内 FileFragment 语义；当前降级不等同于已接入微信本地文件存储。
