...
它本身不扩展新成员，只是把 `IPersistManager` 这套通用持久化读写契约标记为 “PlayerPrefs 存储实现”，供两处使用：

- `PersistComponent` 通过 `PlayerPrefs` 属性暴露该存储实现
- `TypeCreator.Create<IPlayerPrefsManager>(...)` 用它做类型过滤和实例创建

## 什么时候先看这页

- 你要确认 `Nova.Persist.PlayerPrefs` 暴露的到底是什么契约。
- 你要替换 PlayerPrefs 存储实现，但不想改上层调用代码。
...
- 这个接口的意义不是“定义 PlayerPrefs 特有 API”，而是“把某个实现归类为 PlayerPrefs 存储实现”。
- 因为没有附加成员，所以跨存储实现调用面天然一致；真正的差异在具体实现行为，而不在接口签名。

## 与其他存储实现的关系

- `IPlayerPrefsManager`、`IFileFragmentManager`、`ISQLiteManager` 三者都是 `IPersistManager` 的存储实现标记接口。
- 三者可以被 `PersistComponent` 并存持有，但各自面向不同存储模型与平台约束。
- 如果你要写跨存储实现通用逻辑，优先看 `IPersistManager` 语义，不要把这个接口当成扩展点。

## 风险点 / 易错点

- 这个接口不是能力扩展点；给它加成员会直接破坏三种存储实现共用的调用面。
