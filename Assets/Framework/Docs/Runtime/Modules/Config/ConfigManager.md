...
`LoadAsync()` 以 `m_IsLoadOver` 保证幂等。加载完成前先建立本地默认快照并恢复 `persistentDataPath/Nova/Config/app-config.json`；随后立即标记 Config 可用，并异步等待框架内部 `INetworkReadySignal`。标准 Network 路由首次成功构建后自动拉取一次，项目组无需修改 Procedure。
