 ...
 * author:    taoye
 * created:   2026/5/11
 * descrip:   域名表（HostKey）导出工具，封装数据和类型的 Luban 全量导出逻辑
 ***************************************************************/
...
namespace NovaFramework.Editor
{
    public static partial class EditorUtil
    {
        ...
        {
            /// <summary>
            /// 域名表（HostKey）导出工具，封装数据导出和类型代码生成的一键全量导出入口。
            /// </summary>
            public static class HostKeyExporter
            {
                ...
                /// Luban target 名称。
                /// </summary>
                private const string c_TargetName = "network-hostkey";

                /// <summary>
                /// Luban manager 类名。
                /// </summary>
                private const string c_ManagerName = "HostKeyTables";

                /// <summary>
                /// Luban settings 属性名。
                /// </summary>
                private const string c_SettingsPropertyName = "HostKeySettings";

                /// <summary>
                /// 导出域名表所有单元的数据和类型代码（等价于 Inspector "导出所有域名表数据和类型" 按钮）。
                /// </summary>
                /// <param name="settings">域名表设置。</param>
                /// <returns>导出是否成功。</returns>
                public static bool ExportHostKeyAll(HostKeySettings settings)
                {
                    if (settings == null)
                    {
                        Log.Error(LogTag.Editor, "HostKeySettings 为 null，无法导出。");
                        return false;
                    }

                    return ExportAll(settings.SourceDirPath, settings, settings.HostKeyUnits);
                }

                /// <summary>
                /// 仅导出域名表类型代码（跳过数据，重新生成 C# 枚举/常量类）。
                /// </summary>
                /// <param name="settings">域名表设置。</param>
                /// <returns>导出是否成功。</returns>
                public static bool ExportHostKeyCode(HostKeySettings settings)
                {
                    if (settings == null)
                    {
                        Log.Error(LogTag.Editor, "HostKeySettings 为 null，无法导出类型代码。");
                        return false;
                    }

                    return ExportCode(settings.SourceDirPath, settings, settings.HostKeyUnits);
                }

                /// <summary>
                /// 仅导出域名表数据（跳过类型代码，重新生成 JSON 数据文件）。
                /// </summary>
                /// <param name="settings">域名表设置。</param>
                /// <returns>导出是否成功。</returns>
                public static bool ExportHostKeyData(HostKeySettings settings)
                {
                    if (settings == null)
                    {
                        Log.Error(LogTag.Editor, "HostKeySettings 为 null，无法导出数据。");
                        return false;
                    }

                    return ExportData(settings.SourceDirPath, settings, settings.HostKeyUnits);
                }
                ...
                /// <param name="settings">域名表设置（IDataTableSettings）。</param>
                ...
                private static bool ExportAll(string regionDirPath, IDataTableSettings settings, IReadOnlyList<IDataTableUnitSetting> units)
                {
                    ...
                    {
                        Log.Error(LogTag.Editor, "HostKey 数据源目录不存在：{0}", regionDirPath);
                        return false;
                    }

                    if (units == null || units.Count == 0)
                    {
                        Log.Warning(LogTag.Editor, "HostKey 单元设置为空，跳过导出。");
                        return true;
                    }
