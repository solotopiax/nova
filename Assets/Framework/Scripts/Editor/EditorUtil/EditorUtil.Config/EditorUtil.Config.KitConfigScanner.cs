 ...
 * All Rights Reserved.
 * -------------------------------------------------------------
 * filename:  EditorUtil.Config.KitConfigScanner.cs
 * author:    taoye
 * created:   2026/5/31
 * descrip:   反射扫描并管理 IKitConfig 实现类型的工具（三维矩阵存储）
 ***************************************************************/
...
namespace NovaFramework.Editor
{
    public static partial class EditorUtil
    {
        ...
        {
            /// <summary>
            /// 反射扫描所有实现 IKitConfig 的可实例化 [Serializable] 类型，
            /// 并提供对 PlatformChannelEntry 指定 DevelopMode 格 KitConfigs 列表的实例补全与移除操作。
            /// </summary>
            public static class KitConfigScanner
            {
                /// <summary>
                /// 扫描出的 Kit Config 条目，包含类型与左树展示名称。
                /// </summary>
                public readonly struct KitConfigEntry
                {
                    ...
                    /// <summary>
                    /// 左树展示的名称，从实例 DisplayName 读取；空字符串时回退为类型名。
                    /// </summary>
                    public readonly string DisplayName;

                    /// <summary>
                    /// 构造 KitConfigEntry 实例。
                    /// </summary>
                    /// <param name="configType">Kit Config 类型。</param>
                    /// <param name="displayName">左树展示名称。</param>
                    public KitConfigEntry(Type configType, string displayName)
                    {
                ...
                /// <summary>
                /// 扫描全部已加载程序集中符合条件的 Kit Config 类型，并读取实例 DisplayName。
                /// <para>条件：非抽象、非接口、实现 IKitConfig、标注 [Serializable]、有可用无参构造器。</para>
                /// </summary>
                /// <returns>
                /// 满足条件的 KitConfigEntry 列表；若无匹配则返回空列表。
                /// </returns>
                public static List<KitConfigEntry> ScanAll()
                {
                    List<KitConfigEntry> result = new();
                    IEnumerable<Type> types = AppDomain.CurrentDomain.GetAssemblies()
                        .Where(asm => !IsTestAssembly(asm))   // 排除测试程序集（桩类型如 TestKitConfig 不入生产扫描）
                    ...
                    foreach (Type type in types)
                    {
                        ...
                        try
                        {
                            IKitConfig sample = (IKitConfig)Activator.CreateInstance(type);
                        ...
                        result.Add(new KitConfigEntry(type, displayName));
                    }
                    return result;
                }

                /// <summary>
                /// 若指定 DevelopMode 下的 KitConfigs 列表中缺少指定类型的实例，则通过反射补一个空实例并追加至末尾。
                ...
                public static bool EnsureInstance(PlatformChannelEntry entry, DevelopMode mode, Type configType)
                {
                    ...
                    list.Add((IKitConfig)Activator.CreateInstance(configType));
                    return true;
                }

                /// <summary>
                /// 从指定 DevelopMode 下的 KitConfigs 列表中移除所有指定类型的实例。
                ...
                public static void RemoveInstance(PlatformChannelEntry entry, DevelopMode mode, Type configType)
                {
                    ...
                    entry.GetKitConfigs(mode).RemoveAll(c => c != null && c.GetType() == configType);
                }
                ...
                /// 用于把测试桩类型（如 TestKitConfig）排除出生产扫描。
                /// </summary>
