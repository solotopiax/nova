...
namespace NovaFramework.Runtime
{
    ...
    public sealed class NovaLinkedList<T> : ICollection<T>, IEnumerable<T>, ICollection, IEnumerable
    {
        /// <summary>
        /// 内部链表。
        /// </summary>
        ...
        /// <summary>
        /// 缓存结点数量上限，超出的结点在 Clear 时直接丢弃不入缓存，避免内存膨胀。
        /// </summary>
        private const int c_MaxCachedNodeCount = 256;

        /// <summary>
        /// 从链表中移除所有结点。超出缓存上限的结点仍会清除 Value 后直接丢弃，避免引用泄露。
        /// </summary>
        public void Clear()
        {
            ...
            while (current != null)
            {
                ...
                if (m_CachedNodes.Count < c_MaxCachedNodeCount)
                {
                    ReleaseNode(current);
                }
                else
        ...
        /// 清除链表结点缓存。
        /// </summary>
        public void ClearCachedNodes() => m_CachedNodes.Clear();

        /// <summary>
        /// 确定某值是否在链表中。
        /// </summary>
        /// <param name="value"> 指定值。</param>
        /// <returns> 返回某值是否在链表中。</returns>
        public bool Contains(T value) => m_LinkedList.Contains(value);

        /// <summary>
        /// 从目标数组的指定索引处开始将整个链表复制到兼容的一维数组。
        /// </summary>
        /// <param name="array"> 一维数组，它是从链表复制的元素的目标数组必须具有从零开始的索引。</param>
        /// <param name="index"> array 中从零开始的索引，从此处开始复制。</param>
        public void CopyTo(T[] array, int index) => m_LinkedList.CopyTo(array, index);

        /// <summary>
        /// 从特定的 ICollection 索引开始，将数组的元素复制到一个数组中。
        /// </summary>
        /// <param name="array"> 一维数组，它是从 ICollection 复制的元素的目标数组必须具有从零开始的索引。</param>
        /// <param name="index"> array 中从零开始的索引，从此处开始复制。</param>
        public void CopyTo(Array array, int index) => ((ICollection)m_LinkedList).CopyTo(array, index);

        /// <summary>
        /// 查找包含指定值的第一个结点。
        /// </summary>
        /// <param name="value"> 要查找的指定值。</param>
        /// <returns> 返回包含指定值的第一个结点。</returns>
        public LinkedListNode<T> Find(T value) => m_LinkedList.Find(value);

        /// <summary>
        /// 查找包含指定值的最后一个结点。
        /// </summary>
        /// <param name="value"> 要查找的指定值。</param>
        /// <returns> 返回包含指定值的最后一个结点。</returns>
        public LinkedListNode<T> FindLast(T value) => m_LinkedList.FindLast(value);
        ...
        public void RemoveFirst()
        {
            ...
            if (first == null) throw new InvalidOperationException("开头节点无效。");
        ...
        public void RemoveLast()
        {
            ...
            if (last == null) throw new InvalidOperationException("尾部节点无效。");
        ...
        /// 返回循环访问集合的枚举数。
        /// </summary>
        /// <returns> 返回循环访问集合的枚举数。</returns>
        public Enumerator GetEnumerator() => new Enumerator(m_LinkedList);
        ...
        public struct Enumerator : IEnumerator<T>, IEnumerator
        {
            ...
            {
                if (linkedList == null) throw new ArgumentNullException(nameof(linkedList), "链表无效。");
            ...
            /// 重置枚举数。
            /// </summary>
            void IEnumerator.Reset() => ((IEnumerator<T>)m_Enumerator).Reset();
        }
    }
}
