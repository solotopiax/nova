...
namespace NovaFramework.Runtime
{
    ...
    public static class RuntimeDebuggerUtil
    {
        ...
        public static bool EnsureEventSystemExists()
        {
            ...
            Debug.LogWarning("[RuntimeDebugger] No EventSystem found in scene - creating a default one. Disable this behaviour in Window -> RuntimeDebugger -> Settings Window -> Advanced)");
        ...
        {
            var go = new GameObject("EventSystem (Created by RuntimeDebugger, disable in Window -> RuntimeDebugger -> Settings Window -> Advanced)");
            ...
            switch (Settings.Instance.UIInputMode)
            {
                case Settings.UIModes.NewInputSystem:
                    AddInputSystem(go);
                    Debug.LogWarning("[RuntimeDebugger] Automatically generated EventSystem is using Unity Input System (can be changed to use Legacy Input in Window -> RuntimeDebugger -> Settings Window -> Advanced)");
                    break;
                case Settings.UIModes.LegacyInputSystem:
                    AddLegacyInputSystem(go);
                    Debug.LogWarning("[RuntimeDebugger] Automatically generated EventSystem is using Legacy Input (can be changed to use Unity Input System in Window -> RuntimeDebugger -> Settings Window -> Advanced)");
                    break;
            }
        ...
        public static List<OptionDefinition> ScanForOptions(object obj)
        {
            ...
            foreach (var memberInfo in members)
            {
                ...
                var sortAttribute = DebugReflection.GetAttribute<SortAttribute>(memberInfo);
                var sortPriority = sortAttribute == null ? 0 : sortAttribute.SortPriority;

                // Find user-specified display name from attribute
                var nameAttribute = DebugReflection.GetAttribute<DisplayNameAttribute>(memberInfo);
