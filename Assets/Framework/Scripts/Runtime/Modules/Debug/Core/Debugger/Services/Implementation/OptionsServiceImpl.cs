 ...
 * All Rights Reserved.
 * -------------------------------------------------------------
 * filename:  OptionsServiceImpl.cs
 * author:    yingzheng
 * created:   2026/6/12
...
namespace NovaFramework.Runtime
{
    ...
    public partial class OptionsServiceImpl : IOptionsService
    {
        ...
        private void OptionsContainerOnOptionAdded(IOptionContainer container, OptionDefinition optionDefinition)
        {
            ...
            {
                Debug.LogWarning("[RuntimeDebugger] Received event from unknown option container.");
                return;
            }

            if (options.Contains(optionDefinition))
            {
                Debug.LogWarning("[RuntimeDebugger] Received option added event from option container, but option has already been added.");
                return;
            }
        ...
        private void OptionsContainerOnOptionRemoved(IOptionContainer container, OptionDefinition optionDefinition)
        {
            ...
            {
                Debug.LogWarning("[RuntimeDebugger] Received option removed event from option container, but option does not exist.");
            }
        }
