 ...
 * All Rights Reserved.
 * -------------------------------------------------------------
 * filename:  OptionsTabController.cs
 * author:    yingzheng
 * created:   2026/6/12
...
namespace NovaFramework.Runtime
{
    ...
    public class OptionsTabController : DebugMonoBehaviourEx
    {
        ...
        protected void Populate()
        {
            ...
            foreach (var option in Service.Options.Options)
            {
                if (!OptionControlFactory.CanCreateControl(option))
                {
                    ...
                    {
                        Debug.LogError("[NovaFramework.Runtime.OptionsTab] Unsupported property type: {0} (on property {1})".Fmt(option.Property.PropertyType, option.Property));
                    }
                    else
                    {
                        Debug.LogError("[NovaFramework.Runtime.OptionsTab] Unsupported method signature: {0}".Fmt(option.Name));
                    }
                    continue;
                }
        ...
        protected void CreateCategory(string title, List<OptionDefinition> options)
        {
            ...
            var groupInstance = DebugInstantiate.Instantiate(CategoryGroupPrefab);
            ...
            foreach (var option in options)
            {
                ...
                if (control == null)
                {
                    Debug.LogError("[NovaFramework.Runtime.OptionsTab] Failed to create option control for {0}".Fmt(option.Name));
                    continue;
                }
