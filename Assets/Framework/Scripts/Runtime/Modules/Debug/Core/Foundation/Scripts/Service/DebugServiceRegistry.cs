 ...
 * All Rights Reserved.
 * -------------------------------------------------------------
 * filename:  DebugServiceRegistry.cs
 * author:    yingzheng
 * created:   2026/6/12
...
namespace NovaFramework.Runtime
{
    ...
    [AddComponentMenu(ComponentMenuPaths.DebugServiceRegistry)]
    public class DebugServiceRegistry : DebugAutoSingleton<DebugServiceRegistry>
    {
#if DEBUG_FOUNDATION_TRACE
        ...
        private readonly DebugList<Service> _services = new DebugList<Service>();
        ...
        protected void UpdateStubs()
        {
            ...
            RegisterAssembly<DebugServiceRegistry>();
            ...
            foreach (var assembly in _assemblies)
            {
                ...
                {
                    Debug.LogError("[DebugServiceRegistry] Error loading assembly {0}".Fmt(assembly.FullName), this);
            ...
            if (EnableLogging)
            {
                ...
                Debug.Log("[DebugServiceRegistry] Services Discovered: {0} \n  {1}".Fmt(serviceStrings.Length,
                    string.Join("\n  ", serviceStrings)));
            }
        }
        ...
        protected object AutoCreateService(Type t)
        {
            ...
            foreach (var stub in _serviceStubs)
            {
                ...
                if (EnableLogging)
                {
                    Debug.Log("[DebugServiceRegistry] Auto-created service: {0} ({1})".Fmt(stub.Type, stub.InterfaceType),
                        service as Object);
                }
        ...
        protected void OnDisable()
        {
            if (EnableLogging)
            {
                Debug.Log("[DebugServiceRegistry] Cleaning up services");
            }
...
#region Type Scanning
        ...
        {
            var attribute = DebugReflection.GetAttribute<ServiceAttribute>(type);
        ...
        private static void ScanTypeForSelectors(Type t, List<ServiceStub> stubs)
        {
            ...
            {
                var attrib = DebugReflection.GetAttribute<ServiceSelectorAttribute>(method);
        ...
        private static void ScanTypeForConstructors(Type t, List<ServiceStub> stubs)
        {
            ...
            foreach (var method in methods)
            {
                var attrib = DebugReflection.GetAttribute<ServiceConstructorAttribute>(method);
