 ...
 * All Rights Reserved.
 * -------------------------------------------------------------
 * filename:  DebugMonoBehaviourEx.cs
 * author:    yingzheng
 * created:   2026/6/12
...
namespace NovaFramework.Runtime
{
    ...
    /// <summary>
    /// Add to a field to attempt to use DebugServiceRegistry to get an instance of the field type
    /// </summary>
    ...
    public abstract class DebugMonoBehaviourEx : DebugMonoBehaviour
    {
        private static Dictionary<Type, IList<FieldInfo>> _checkedFields;

        private static void CheckFields(DebugMonoBehaviourEx instance, bool justSet = false)
        {
        ...
        private static void PopulateObject(IList<FieldInfo> cache, DebugMonoBehaviourEx instance, bool justSet)
        {
            for (var i = 0; i < cache.Count; i++)
            {
                ...
                // If import is enabled, use DebugServiceRegistry to import the reference
                if (f.Import)
                {
                    var t = f.ImportType ?? f.Field.FieldType;

                    var service = DebugServiceRegistry.GetService(t);
        ...
        private static List<FieldInfo> ScanType(Type t)
        {
            ...
            var globalAttr = DebugReflection.GetAttribute<RequiredFieldAttribute>(t);
		...
		var fields = t.GetTypeInfo().DeclaredFields.Where(f => !f.IsStatic);
            ...
            {
                var requiredFieldAttribute = DebugReflection.GetAttribute<RequiredFieldAttribute>(f);
                var importAttribute = DebugReflection.GetAttribute<ImportAttribute>(f);
