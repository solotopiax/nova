...
- 平台参数 GUID（32 hex = 16 字节）按 **8/8/16** 定长打包三值：
  - `[0,8)` **uid**：8 位十六进制字符串，原文左补 0（业务约束：空值或 1-8 位十六进制，非空不能以 0 开头）。
  - `[8,16)` **tableId**：4 字节，数值左补 0（业务约束正数、≤8 位十进制）。
  - `[16,32)` **receiptParam**：16 位十六进制字符串，原文左补 0（业务约束：空值或 1-16 位十六进制，非空不能以 0 开头）。
- uid / receiptParam 不要求能解析为 `long`，但必须是合法十六进制，以保证整体是 iOS 可接受的 GUID；非空前导 `0` 会与补位零混淆，因此直接拒绝。tableId 仍按数值编码。
- `ReceiptParam` 作为独立字段新增在 `IAPRequest` / `IAPResult`（`string` 类型），与自由格式、仅本地往返的 `CustomData` 分离；`IAPResult.ReceiptParam` 为只读、经构造函数回填。
- 范围 / 格式校验统一在 `MobilePurchaseService.TryValidatePassthroughParams`，由 `PayAsync` 入口调用。tableId 越界、uid / receiptParam 超槽位长度、含非十六进制字符或以 `0` 开头的非空值都直接拒绝支付（`IAPMobileErrorCode.InvalidPassthroughParam`）。`ApplyPurchaseContext` / codec 只做纯编码，不重复校验。
...
- 字段容量上限为：uid 为 1-8 位十六进制、tableId ≤8 位十进制、receiptParam 为 1-16 位十六进制；uid / receiptParam 非空值不能以 `0` 开头。业务侧如需自由格式、更长的值或保留前导零，需要另开版本化编码并配合服务端映射。
