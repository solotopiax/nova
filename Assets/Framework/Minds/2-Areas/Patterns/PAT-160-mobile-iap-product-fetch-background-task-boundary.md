...
把商品拉取做成“成功态单向收敛 + 失败态有限重试 + 不可用 SKU 按 Controller 事实校正”，可以同时覆盖全失败、部分成功和迟到失败三类场景。启动期和订阅倒计时只做无系统登录弹框的 `FetchPurchases` 和权益刷新，用户主动恢复购买才调用 `RestoreTransactions`，既避免 iOS 无交互弹框，也保留订阅和非消耗品恢复链路。后台任务入口只承担取消和异常收口，可以避免补单、Restore 和权益刷新各自裸 `Forget()`，也避免有返回值方法被当成无返回后台任务直接传入导致编译或语义错误。
