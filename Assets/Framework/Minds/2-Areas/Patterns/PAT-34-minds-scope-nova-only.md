...
日常任务可以把高置信、长期有效且证据完整的 Nova Framework 知识直接写入正式区，不经过草稿目录。自动入库不改变本 Pattern 的内容边界：协作工具流程、临时状态和会话流水账仍不得进入 `Minds`。

防重命中或需要修改、覆盖、合并、归档已有条目时，必须先向用户确认，不能用新增近义条目规避治理。
