...
| 已冻结的单文件或完整区域导出范围 | `EditorUtil.Vibrate.ExportEmphasisData` / `ExportEmphasisCode` / `ExportEmphasisAll`；`ExportCustomData` / `ExportCustomCode` / `ExportCustomAll` | 指定区域的数据、类型和映射产物 | 导出事务成功，变更仅位于冻结输出集；另一地区域未被修改 |
| 已确认的全 Emphasis 或全 Custom 区域范围 | `export.vibrate.emphasis.data` / `export.vibrate.emphasis.code` / `export.vibrate.custom.data` / `export.vibrate.custom.code` | 对应区域完整数据或类型产物 | 当前活动场景有 `Nova` 与 `VibrateComponent`，Step 成功或在首个失败处停止 |
...
2. 仅更新冻结的区域数据、Unit / 资源映射和既有业务入口。先检查每个区域内的 `Name`、`Order` 和字段完整性，再选择精确单文件 Exporter 或已确认的区域批次；导出失败立即停止，不清理另一地区域或无关生成物。
