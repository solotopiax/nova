...
先读取 `references/contract.json` 与随包 `Docs/START_HERE.md`，只检查用户声明的项目范围。该 Skill 只报告，不安装包、不修复配置、不删除缓存。

## 检查步骤

1. 用 `nova_skills.py resolve` 确认消费项目实际解析到的 Framework 包；记录 Unity 版本、Framework 版本、目标平台、当前场景和 Build Settings 的可见事实。
